...
# stores default configurations for multiple ViT variants
default_cfgs = {
...
class Q_Mlp(nn.Module):
    ...
    def __init__(
        ...
        self.fc1 = LinearQ(
            in_features, hidden_features, nbits_w=nbits, mode=Qmodes.kernel_wise
        )
        self.act = act_layer()
        self.drop1 = nn.Dropout(drop_probs[0])
        self.fc2 = LinearQ(
            hidden_features, out_features, nbits_w=nbits, mode=Qmodes.kernel_wise
        )
...
class Q_Attention(nn.Module):
    ...
    def __init__(
        ...
        if self.quantize_attn:
            ...
            self.qkv = LinearQ(
                dim, dim * 3, bias=qkv_bias, nbits_w=nbits, mode=Qmodes.kernel_wise
            )
...
class lowbit_VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        # Classifier head(s)
        self.head = (
            LinearQ(self.num_features, num_classes, nbits_w=8)
            if num_classes > 0
            else nn.Identity()
        )
        ...
        if distilled:
            self.head_dist = (
                LinearQ(self.embed_dim, self.num_classes, nbits_w=8)
                if num_classes > 0
                else nn.Identity()
            )
